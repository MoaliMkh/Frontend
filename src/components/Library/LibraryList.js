...
import no_content from "../../images/no-content.png";
...
const LibraryList = (props) => {
  ...
  const fetchEachLibrary = async (index, type) => {
    ...
    try {
      const res = await req.get(`/${user_id}/library/${index}/file/`, {
  ...
  const deleteEachLibrary = async (index) => {
    ...
    try {
      await req.delete(`/${user_id}/library/${index}/`, {
        headers: { Authorization: `Token ${token}` },
      });
    ...
    return (
      <li style={{ width: "80%", marginBottom: "15%" }} key={idx}>
        <div class="max-w-sm rounded overflow-hidden shadow-lg">
          <div
            onClick={() => {
              fetchEachLibrary(idx, library.content_type);
            }}
            class="bg-sky-700"
          >
            <img
          ...
          <div class="px-6 bg-zinc-300 flex">
            ...
            <span
              class="inline-block bg-zinc-400 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2"
              onClick={() => {
                deleteEachLibrary(idx);
              }}
            >
              حذف
            </span>
      ...
      <FlatList
        ...
        renderWhenEmpty={() => (
          <div style={{ color: "red", textAlign: "center" }}>
            <span>کتابخانه‌ای برای نمایش وجود ندارد</span>
